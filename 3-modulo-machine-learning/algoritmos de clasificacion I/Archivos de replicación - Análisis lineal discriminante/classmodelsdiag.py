...
"""
File: classmodelsdiag.py
Author: Ignacio Soto Zamorano
Email: ignacio[dot]soto[dot]z[at]gmail[dot]com
Github: https://github.com/ignaciosotoz/ml-diagnosis
Description: Assorted functions for classification models
"""
...
class ClassificationModelMetrics(object):
    ...
    def __hyperparams_dictionary(self, model_name):
        ...
        tmp = {
            ...
            'SVC': ['gamma', np.logspace(-1, -6, 50, dtype=float), "semilog"],
    ...
    def __init__(self, model, X_mat, y_vec, test_size_percentage=.33, random_seed=11238):
        ...
        self.X_mat = X_mat
        self.y_vec = y_vec
        self.y_vec_labels = y_vec.unique()
    ...
    def confusion_plot(self):
        ...
        """
        y_hat = self.model.fit(self.X_mat_train, self.y_vec_train).predict(self.X_mat_test)
        tmp_confused = confusion_matrix(self.y_vec_test, y_hat) / len(self.y_vec_test)
        custom_cmap = LinearSegmentedColormap.from_list('lista', color_palette_sequential)
        sns.heatmap(tmp_confused,xticklabels=self.y_vec_labels, yticklabels=self.y_vec_labels,
                    annot = True, fmt='.1%', cbar=False, cmap=custom_cmap)
        plt.grid(False)
    ...
    def plot_class_report(self):
        ...
        """
        y_hat = self.model.fit(self.X_mat_train, self.y_vec_train).predict(self.X_mat_test)
        tmp_report = classification_report(self.y_vec_test, y_hat).split()
        tmp_report = [i for i in tmp_report if i not in ['avg', '/', 'precision', 'recall', 'f1-score', 'support']]
        tmp_report = pd.DataFrame(np.array(4, len(tmp_report)/4))
        ...
        for index, (colname, serie) in enumerate(tmp_report.iteritems()):
            ...
            plt.barh(range(tmp_report.shape[0] - 1), serie.drop('total'), color='slategrey')
            if index + 1 is not tmp_report.shape[0]:
                plt.axvline(serie.iloc[-1], color='tomato', linestyle='--')
            plt.title(colname)
            plt.tight_layout()
